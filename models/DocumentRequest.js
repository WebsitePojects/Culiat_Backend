    ...
    url: {
      ...
      validate: {
        validator: function (v) {
          ...
          return /^(https?:\/\/.+|\/uploads\/.+|\/.+\.(jpg|jpeg|png))$/i.test(
            v
          );
        },
...
const documentRequestSchema = new mongoose.Schema(
  {
    ...
    // Control number - generated on approval
    controlNumber: {
      type: String,
      unique: true,
...
documentRequestSchema.pre("save", function (next) {
  ...
  if (this.validID && this.validID.url) {
    ...
    if (!validation.isValid) {
      return next(
        new Error(`Valid ID validation failed: ${validation.errors.join(", ")}`)
      );
    }
  }
  ...
  if (this.photo1x1 && this.photo1x1.url) {
    ...
    if (!validation.isValid) {
      return next(
        new Error(
          `Photo 1x1 validation failed: ${validation.errors.join(", ")}`
        )
      );
    }
  }
...
documentRequestSchema.statics.createFromUser = async function (
  userId,
  requestData
) {
...
// Control number prefixes by document type
const CONTROL_NUMBER_PREFIXES = {
  indigency: "IND",
  residency: "RES",
  clearance: "CLR",
  good_moral: "GMC",
  business_permit: "BPM",
  business_clearance: "BCL",
  barangay_id: "BID",
  liquor_permit: "LQR",
  missionary: "MIS",
  rehab: "REH",
...
// Static method to generate sequential control number
documentRequestSchema.statics.generateControlNumber = async function (
  documentType
) {
  const year = new Date().getFullYear();
  const prefix = CONTROL_NUMBER_PREFIXES[documentType] || "DOC";

  // Find the last control number for this type and year
  const lastDoc = await this.findOne({
    controlNumber: new RegExp(`^${prefix}-${year}-`),
  }).sort({ controlNumber: -1 });

  let sequence = 1;
  if (lastDoc && lastDoc.controlNumber) {
    const parts = lastDoc.controlNumber.split("-");
    if (parts.length === 3) {
