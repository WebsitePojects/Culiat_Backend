...
exports.getPaymentDetails = async (req, res) => {
  try {
    ...
    const isOwner =
      documentRequest.applicant?._id?.toString() === req.user._id.toString();
    const isAdmin =
    ...
    res.status(200).json({
      ...
      data: {
        ...
        documentLabel:
          DOCUMENT_LABELS[documentRequest.documentType] ||
        ...
        applicant: {
          firstName:
            documentRequest.firstName || documentRequest.applicant?.firstName,
          lastName:
            documentRequest.lastName || documentRequest.applicant?.lastName,
          email:
            documentRequest.emailAddress || documentRequest.applicant?.email,
          phone:
            documentRequest.contactNumber ||
            documentRequest.applicant?.phoneNumber,
        },
...
exports.createPaymentLink = async (req, res) => {
  try {
    ...
    const documentLabel =
      DOCUMENT_LABELS[documentRequest.documentType] ||
      documentRequest.documentType;
    const description = `${documentLabel} (₱${basePrice} + ₱${commission.toFixed(
      2
    )} fee)`;
  ...
  try {
    ...
    if (documentRequest.paymentReference) {
      try {
        ...
        const sessionStatus =
          response.data.data.attributes.payment_intent?.attributes?.status;
        ...
        if (sessionStatus === "succeeded") {
          ...
          await logAction(
            LOGCONSTANTS.actions.records?.PAYMENT_COMPLETED ||
              "PAYMENT_COMPLETED",
...
exports.waivePayment = async (req, res) => {
  try {
    ...
    await logAction(
      ...
      `Payment waived for request: ${requestId}. Reason: ${
