...
const verifyByControlNumber = async (req, res) => {
  try {
    ...
    // Allow both 'approved' and 'completed' status for verification (same as token verification)
    if (!['approved', 'completed'].includes(document.status)) {
      return res.status(200).json({
        success: true,
        verified: false,
    ...
    // Build resident name from document fields (consistent with token verification)
    const residentName = [
      document.firstName,
      document.middleName,
      document.lastName,
